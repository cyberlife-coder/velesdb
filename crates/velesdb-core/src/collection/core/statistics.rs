...
impl Collection {
    ...
    pub fn analyze(&self) -> Result<CollectionStats, Error> {
        ...
        let hnsw_stats = IndexStats::new("hnsw_primary", "HNSW").with_entry_count(hnsw_len as u64);
        collector.add_index_stats(hnsw_stats);

        // BM25 index statistics - use len() if available
        let bm25_len = self.text_index.len();
        if bm25_len > 0 {
            let bm25_stats = IndexStats::new("bm25_text", "BM25").with_entry_count(bm25_len as u64);
