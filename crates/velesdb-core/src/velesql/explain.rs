...
use crate::collection::search::query::match_planner::{CollectionStats, MatchExecutionStrategy, MatchQueryPlanner};
...
impl QueryPlan {
    ...
    pub fn from_match(match_clause: &MatchClause, stats: &CollectionStats) -> Self {
        ...
        let (start_labels, max_depth, has_similarity, similarity_threshold) = match &strategy {
            ...
            MatchExecutionStrategy::Parallel { graph_hint, vector_hint } => {
        ...
        let index_used = if has_similarity { Some(IndexType::Hnsw) } else { None };
...
mod tests {
    ...
    // EPIC-046 US-004: EXPLAIN MATCH tests
    #[test]
    fn test_match_traversal_plan_node() {
        ...
        assert!(cost > 0.1); // base cost
        assert!(cost < 1.0); // not too expensive
    }
